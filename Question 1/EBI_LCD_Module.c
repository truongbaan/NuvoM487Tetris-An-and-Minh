...
uint16_t Get_TP_X(void)
{
    ...
    EADC->CTL |= (0x00000002UL << 2);	//Enable sample module A/D ADINT1 interrupt. 
		EADC->INTSRC[1] |= 0x00000002UL;	//Enable sample module 1 interrupt.
		NVIC->ISER[1] |= (1 << (43 - 32));
...
uint16_t Get_TP_Y(void)
{
    ...
    /* Enable the sample module 2 interrupt. */  
		EADC->CTL |= (0x00000004UL << 2);	//Enable sample module A/D ADINT1 interrupt.   
    EADC->INTSRC[1] |= 0x00000004UL;	//Enable sample module 2 interrupt.
		NVIC->ISER[1] |= (1 << (46 - 32));
